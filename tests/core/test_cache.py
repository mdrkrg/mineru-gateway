...
def _store_via_writer(cache: FileCache, data: dict, files: list) -> str:
    ...
    return writer.finish(data)


async def test_store_returns_existing_dir(tmp_path):
    cache = FileCache(str(tmp_path))
    cache_dir = _store_via_writer(cache, {"backend": "pipeline"}, _files())
...
async def test_restore_roundtrips_form_and_files(tmp_path):
    ...
    cache_dir = _store_via_writer(cache, data, _files())
...
async def test_release_removes_dir(tmp_path):
    cache = FileCache(str(tmp_path))
    cache_dir = _store_via_writer(cache, {}, _files())
